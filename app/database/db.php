...
function selectSectionOnAbout($table)
{
    global $pdo;
    $sql = "SELECT 
        t1.id,
...
function selectRecentsPostOnBlog($table1, $table2, $id)
{
    global $pdo;
    $sql = "SELECT p.*, u.username FROM $table1 AS p JOIN $table2 AS u ON p.id_user = u.id WHERE p.id=$id LIMIT 4";
    $query = $pdo->prepare($sql);
    $query->execute();
    // dbCheckError($query);
    return $query->fetchAll();
...
function selectTopicsPostsOnSigle($table1, $table2, $id)
{
    global $pdo;
    $sql = "Select p.id, p.title,b.name From $table1 p Join $table2 AS b ON p.id_topic = b.id WHERE p.id = $id";
    $query = $pdo->prepare($sql);
    $query->execute();
    // dbCheckError($query);
    return $query->fetch();
